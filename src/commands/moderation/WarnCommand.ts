...
export class WarnCommand extends BaseCommand {
    public async execute(message: IMessage, args: string[]): Promise<any> {
        ...
        if (!user || ((args[0] as string|undefined)?.replace(/[^0-9]/g, "") !== user.id)) return message.channel.send(createEmbed("error", "Invalid user"));
        ...
        return message.channel.send(createEmbed("success").setTitle(`${user.username} has been warned!`).addField("Reason", reason ?? "No Reason")
