...
import { Util, VoiceChannel, Message } from "discord.js";
...
export class PlayCommand extends BaseCommand {
    ...
    public async execute(message: IMessage, args: string[]): Promise<any> {
        ...
        if (message.guild?.queue && voiceChannel.id !== message.guild!.queue!.voiceChannel?.id) return message.channel.send({ embeds: [createEmbed("error", `I'm currently playing music in \`${message.guild!.queue!.voiceChannel!.name}\` voice channel.`)] });
        ...
        try {
            ...
            try {
                ...
                const msg = await message.channel.send({
                    embeds: [createEmbed("info", `\`\`\`${videos.map((x, i) => `${i + 1} - ${Util.escapeMarkdown(decodeHTML(x.title!))}`).join("\n")}\`\`\`\nPlease, select one of the results.`)
                ...
                const response = await message.channel.awaitMessages({
                    ...
                    max: 1,
                    time: 15000,
                    filter: (m: Message) => (message.author.id === m.author.id) && (m.content === "cancel" || (Number(m.content) >= 1 && Number(m.content) <= videos.length))
                }).catch(() => undefined);
                ...
                if (response?.first()?.content === "cancel") return message.channel.send({ embeds: [createEmbed("warn", "Music selection canceled")] });
    ...
    private async handleVideo(video: Video, message: IMessage, voiceChannel: VoiceChannel): Promise<any> {
        ...
        //     message.channel.send({ embeds: [createEmbed("info", `**[${song.title}](${song.url})** has been added to the queue`).setThumbnail(song.thumbnail)] }).catch(() => null);
        // } else {
