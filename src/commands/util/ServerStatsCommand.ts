...
export class ServerStatsCommand extends BaseCommand {
    public readonly options: Record<string, (message: IMessage, args: string[]) => any> = {
        setup: async (message: IMessage) => {
            ...
            if (!message.member!.permissions.has("MANAGE_CHANNELS")) return message.channel.send({ embeds: [createEmbed("error", "You don't have `Manage Channels` permission to use this command!")] });
            if (!message.guild!.me!.permissions.has("MANAGE_CHANNELS")) return message.channel.send({ embeds: [createEmbed("error", "I don't have `Manage Channels` permission to setup member counter channel!")] });
            if ((await this.client.db.collection("membercounter").findOne({ guild: message.guild!.id }))) return message.channel.send({ embeds: [createEmbed("error", "You already have a stats running.")] });

            const msg = await message.channel.send({ embeds: [createEmbed("info", "Setting up member counter channel...")] });
            const category = await message.guild!.channels.create("Server Stats", {
                type: "GUILD_CATEGORY"
            }).catch(() => undefined);
            if (!category) return message.channel.send({ embeds: [createEmbed("error", "An error occured while trying to make counter category channel. Please, try again later.")] });
            ...
            await msg.edit({ embeds: [createEmbed("success", `Successfully created a server stats! Use \`${this.client.config.prefix}serverstats edit\` to edit the server stats.`)] });
        },
        help: async (message: IMessage) => {
            if (!message.member!.permissions.has("MANAGE_CHANNELS")) return message.channel.send({ embeds: [createEmbed("error", "You don't have `Manage Channels` permission to use this command!")] });
            await message.channel.send({
                embeds: [createEmbed("info", "Option list").setTitle("Server stats utility").addFields([
                    {
                        name: "setup",
        ...
        list: async (message: IMessage) => {
            ...
            if (!data) return message.channel.send({ embeds: [createEmbed("error", `There's no server stats running in your server. Use \`${this.client.config.prefix}serverstats setup\` to create one.`)] });

            return message.channel.send({ embeds: [createEmbed("info", data.channels.length <= 0 ? "None." : `Here's the list of statistic shown by the server stats. If you want to delete a statistic, just delete the statistic VC.\n\n${data.channels.map((x, i) => `${i + 1}. - Format: \`${x.format}\`. Preview: \`${this.client.memberCounter.parseString(x.format, message.guild!)}\``).join("\n")}`)] });
        },
        add: async (message: IMessage, args: string[]) => {
            ...
            if (!args.length) return message.channel.send({ embeds: [createEmbed("error", "Please, give the statistic format text.")] });
            ...
            collector.on("end", async (collected: Collection<string, MessageReaction>, reason: string) => {
                if (reason === "continue") {
                    ...
                    await previewmsg.edit({ embeds: [createEmbed("info", "Adding...")] }).catch(() => null);
                    setTimeout(async () => {
                        const newdata = await collection.findOne({ guild: message.guild!.id });
                        if (!newdata) return previewmsg.edit({ embeds: [createEmbed("error", "Unable to add statistic because the server stats data of this server was deleted.")] });
                        ...
                        return previewmsg.edit({ embeds: [createEmbed("success", "Statistic added!")] });
                    }, 1000);
                } else if (reason === "cancel") {
                    await previewmsg.edit({ embeds: [createEmbed("error", "Canceled")] });
                }
            });
        }
    };
