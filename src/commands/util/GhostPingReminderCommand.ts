...
export class GhostPingReminderCommand extends BaseCommand {
    public readonly options: Record<string, (message: IMessage, args: string[]) => any> = {
        disable: async (message: IMessage) => {
            ...
            if (!data?.enabled) return message.channel.send(createEmbed("error", "Ghost ping reminder is already disabled"));

            data.enabled = false;
            await this.collection.updateOne({ guild: message.guild!.id }, { $set: data }, { upsert: true });
            return message.channel.send(createEmbed("success", "Ghost ping reminder disabled"));
        },
        enable: async (message: IMessage) => {
            ...
            if (data) {
                if (data.enabled) return message.channel.send(createEmbed("error", "Ghost ping reminder is already enabled"));
            ...
            return message.channel.send(createEmbed("success", "Ghost ping reminder enabled"));
        },
        "whitelist-channel": async (message: IMessage, args: string[]) => {
            ...
            const data = await this.collection.findOne({ guild: message.guild!.id });
            if (!data) return message.channel.send(createEmbed("error", "Ghost ping reminder data of this server couldn't be found"));

            data.whitelistedChannels.push(channel.id);
            await this.collection.updateOne({ guild: message.guild!.id }, { $set: data }, { upsert: true });
            return message.channel.send(createEmbed("success", `\`${channel.name}\` channel is whitelisted`));
        },
        "whitelist-user": async (message: IMessage, args: string[]) => {
            ...
            if (!user) return message.channel.send(createEmbed("error", "Invalid user"));
            ...
            await this.collection.updateOne({ guild: message.guild!.id }, { $set: data }, { upsert: true });
            return message.channel.send(createEmbed("success", `${user.tag} is whitelisted`));
        }
    };
