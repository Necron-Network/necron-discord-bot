...
export class MemberCounterManager {
    ...
    public parseString(str: string, guild: IGuild): string {
        return str.replace(/{guild.member.count}/g, guild.members.cache.size.toString())
            ...
            .replace(/{guild.voice.count}/g, guild.channels.cache.filter(c => c.type === "GUILD_VOICE").size.toString())
            .replace(/{guild.text.count}/g, guild.channels.cache.filter(c => c.type === "GUILD_TEXT").size.toString())
            .replace(/{guild.category.count}/g, guild.channels.cache.filter(c => c.type === "GUILD_CATEGORY").size.toString());
    }
}
